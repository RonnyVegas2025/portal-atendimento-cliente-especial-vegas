...
function NovoAtendimentoContent() {
  ...
  const [form, setForm] = useState({ ...initialForm, company_id: companyIdFromQuery || "" });
  ...
  useEffect(() => {
    if (companyIdFromQuery) {
      setForm((prev) => ({ ...prev, company_id: companyIdFromQuery }));
    }
  }, [companyIdFromQuery]);
  ...
  async function handleSubmit(e) {
    ...
    if (error) {
      ...
      setSucesso(`Atendimento criado com protocolo ${data?.protocol || "-"}.`);
      setTimeout(() => router.push("/atendimentos"), 1200);
    }
  ...
  return (
    <AppShell title="Novo atendimento" subtitle="Vincule o atendimento a uma empresa já cadastrada e gere o protocolo.">
      <Card>
        <form onSubmit={handleSubmit} className="p-5 grid md:grid-cols-2 gap-4">
            ...
            <Select value={form.company_id} onChange={(e) => handleChange("company_id", e.target.value)}>
              ...
              {empresas.map((empresa) => (
                <option key={empresa.id} value={empresa.id}>
                  {empresa.trade_name} {empresa.cnpj ? `- ${empresa.cnpj}` : ""}
                </option>
              ))}
            </Select>
          </div>
          ...
          <div>
            ...
            <Input value={form.requester_name} onChange={(e) => handleChange("requester_name", e.target.value)} />
          </div>

          <div>
            <Label>Nome do funcionário</Label>
            <Input value={form.employee_name} onChange={(e) => handleChange("employee_name", e.target.value)} />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              className="w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-slate-300 min-h-[140px]"
              value={form.description}
              onChange={(e) => handleChange("description", e.target.value)}
              placeholder="Exemplo: segunda via do cartão do colaborador..."
            />
          </div>
